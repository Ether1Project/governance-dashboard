         ...
         if(!error)
         {
           if(result)
           {
               ...
               } else
               {
                   ...
                   //var formText = '<form id="subscribe-form"><div class="form-group group-inline"><label for="subscribe-email" class="form-label">Your ETHO Address*</label><input type="email" id="subscribe-email" class="form-control form-primary form-animated" name="subscribe-email" required><button type="submit" class="btn btn-primary">Signup</button></div><p class="m-b-0 fade-color-3"><small>Click Signup to Register Your Metamask/Ether-1 Browser Extension Address With ethoFS To Continue</small></p></form></div></div>';
                   ...
                   document.getElementById("ethoFSLogin").innerHTML = formText;
                   //document.getElementById('etho-address').value = web3.eth.defaultAccount;
                   //$('#etho-address').val(web3.eth.defaultAccount);
                   //console.log("ETHO Address: " + web3.eth.defaultAccount);
